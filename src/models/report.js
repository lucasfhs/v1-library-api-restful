...
class Report {
  ...
  static async getHighAvailabilityCatalog() {
    const query = `
      SELECT
	      L.id,
	      titulo,
      ...
      LEFT JOIN biblioteca B ON R.id_biblioteca = B.id
      GROUP BY L.id, titulo, autor, categoria, paginas, preco, idioma
      HAVING COALESCE(SUM(quantidade_disponivel), 0) > 10;
    `;
    try {
      const result = await client.query(query);
      return result.rows;
    } catch (error) {
      console.error("Error fetching high available books:", error);
      throw error;
    }
  }

  static async getMatureCustomers() {
    const query = `
      SELECT
	      e.ID AS Emprestimo_ID,
	      l.Titulo AS Livro,
      ...
      WHERE e.cpf_usuario IN (
	      SELECT CPF
	      FROM Usuario
	      WHERE DATE_PART('year', AGE(Data_Nascimento)) > 25
        )
      ORDER BY e.Data_Devolucao;
    `;
    try {
